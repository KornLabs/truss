...
> Dies ist die deutsche Übersetzung der [englischen README](README.md) (Stand: `1.0.0`). Bei Abweichungen ist die englische Version maßgeblich.
...
`.truss/` ist **ausführbarer Code**, keine Dokumentation: `init` und `doctor` sind Node-Skripte, die du ausführst — und die dein Agent ausführt. Der Clone unten ist auf einen Release-Tag gepinnt statt auf `main`; `main` ist der Entwicklungszweig und kann unveröffentlichte Änderungen tragen. Lies den Ordner, bevor du ihn ausführst, wie bei jeder anderen Abhängigkeit auch.
...
git clone --depth 1 --branch v1.0.0 https://github.com/KornLabs/truss.git /tmp/truss
...
git clone --depth 1 --branch v1.0.0 https://github.com/KornLabs/truss.git $env:TEMP\truss
...
4. **Strukturierte IDs, nie wiederverwendet.** `D-NNN` Entscheidungen, `OD-NNN` offene Fragen, `HT-NNN` Human-To-dos, `R-NNN` Risiken, `L-NNN` Learnings, `TF-NNN` Befunde über Truss selbst (per `--findings off` abwählbar). Entscheidungen werden supersedet, nie gelöscht. Jede Behauptung im Workspace führt zu genau einem nummerierten Eintrag samt Begründung zurück. Der Satz gehört dir: `docs/schema.md` trägt je Klasse eine Zeile — Datei, Form, Pflichtfelder — und genau diese Tabelle lesen die Prüfungen. Eine eigene Klasse `BL-NNN` für einen Backlog ist eine Zeile, kein Fork.
...
Die CLI existiert primär für den Agenten. Deine Schnittstelle sind die Dateien selbst: schlichtes Markdown, lesbar in jedem Editor, auf GitHub oder im Pull-Request-Diff — `state/current.md` für den Stand der Arbeit, `HUMAN-TODOS.md` für das, was dich braucht, `state/open-decisions.md` für das, was auf deine Entscheidung wartet. Zwei Befehle decken den Rest ab: `truss status` für einen Fünf-Zeilen-Überblick, `truss doctor` für die Frage, ob der Workspace noch mit sich selbst übereinstimmt.
...
`1.0.0` — Datei-Grammatik und CLI sind ab hier ein versionierter Vertrag: Breaking Changes nur mit Major-Sprung und Migrationsnote, additive als Minor-Releases.

...
import { SEG_COLORS, THRESHOLDS, budgetStatus, CLEANUP_PROMPT } from '../context-config.js';
    ...
    return html`
        <${Card}>
          ...
          <div class="row between" style="font-size:10.5px;color:var(--text-3);margin-top:4px">
            <span style=${`color:var(--ok);font-weight:${stat.tone === 'ok' ? '700' : '400'};opacity:${stat.tone === 'ok' ? '1' : '0.6'}`}>healthy ${(THRESHOLDS.floor / 1000).toFixed(1)}–${THRESHOLDS.green / 1000}k</span>
          ...
          <p class="dim" style="font-size:11px;margin-top:12px;line-height:1.5">
            ...
            number matches <code>truss doctor</code>). Task-selected domain and source context are not counted.
            Why it matters: model quality decays as input grows ("context rot", Chroma 2025) — a lean boot
            context leaves more room for the task.</p>
        <//>

...
test('config: bands are floor 4.6k / green 18k / yellow 30k', () => {
  assert.equal(TRUSS_BASELINE, 4611);
  assert.equal(THRESHOLDS.floor, 4611);
  assert.equal(THRESHOLDS.green, 18000);
  assert.equal(THRESHOLDS.yellow, 30000);
  assert.equal(budgetStatus(4611).tone, 'ok');

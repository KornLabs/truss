...
test('config: bands are floor 2.7k / green 18k / yellow 30k', () => {
  assert.equal(TRUSS_BASELINE, 2741);
  assert.equal(THRESHOLDS.floor, 2741);
  assert.equal(THRESHOLDS.green, 18000);
  assert.equal(THRESHOLDS.yellow, 30000);
  assert.equal(budgetStatus(2741).tone, 'ok');

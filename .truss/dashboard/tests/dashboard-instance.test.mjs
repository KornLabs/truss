...
test('config: bands are floor 3.6k / green 18k / yellow 30k', () => {
  assert.equal(TRUSS_BASELINE, 3645);
  assert.equal(THRESHOLDS.floor, 3645);
  assert.equal(THRESHOLDS.green, 18000);
  assert.equal(THRESHOLDS.yellow, 30000);
  assert.equal(budgetStatus(3645).tone, 'ok');

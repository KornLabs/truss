...
2. Redesign the future phases: add, split, drop, reorder, or rewrite. Keep the plan linear and small (3–6 phases total); per phase keep `purpose`, `behavior`, and machine-checkable `exit` criteria concrete. `forbidden`/`forbidden-globs` stay honest — they are enforced guardrails, not decoration.

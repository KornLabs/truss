...
**4. Tailor the phase plan (→state/phases.md).** The installed `kickoff` phase is a seed, not the plan — leaving it in place is an explicit `forbidden` of this phase. From the vision, derive the lifecycle THIS project actually needs: rename phases to the project's language, drop what doesn't apply, add or split what's missing (e.g. a launch, migration, research, or operate phase). Keep it linear and small (3–6 phases); per phase, make `purpose`, `behavior`, and machine-checkable `exit` criteria concrete for this project — vague criteria make every later gate worthless. Keep each phase's `forbidden`/`forbidden-globs` honest: they are advisory safety rails backed by changed-path reports where git evidence is available. Record the tailored plan's rationale as a D-NNN.

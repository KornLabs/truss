...
**5. Phase model — fit the project, then confirm.** The installed `ingest→operate` is a default, not a constraint. From maturity (intake + survey), propose the lifecycle for the human to approve: keep `ingest→operate` if it genuinely fits, or author a bespoke `state/phases.md` — as few as one phase — matching how this project actually runs. Reuse existing kickoff prompts or omit `prompts:`, and keep `doctor` (RF-04) clean. Stamp any bespoke list with `profile: custom` and a one-line top comment ("project-specific phases, authored during overlay ingest <date>, rationale: …") so future agents know it's bespoke and why. Advancing `current:` stays the human's call (AGENTS.md §4).

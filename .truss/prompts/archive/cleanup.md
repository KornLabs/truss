...
Scope is the always-loaded set (AGENTS.md §1): AGENTS.md, state/current.md, VISION.md, state/decisions.md, state/open-decisions.md, state/profile.md, plus the current phase's `read:` targets. Judge each block against the admission test in docs/protocols.md — **what does a future session do differently because this is here?** — and against the current phase, not the project's history. Relevance decides, never age.
